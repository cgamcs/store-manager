    ...
    <ComboboxPrimitive.Portal>
      ...
      >
        <ComboboxPrimitive.Popup
          ...
          // Detiene la propagación del evento para que el Dialog no detecte un clic fuera
          onPointerDown={(e) => e.stopPropagation()}
          onKeyDown={(e) => e.stopPropagation()}
          className={cn(
            "bg-popover text-popover-foreground rounded-md shadow-md ring-1 ring-foreground/10 outline-none",
            "data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95",
            "w-(--anchor-width) min-w-[200px] overflow-hidden",
            className
          )}
          {...props}
        />
...
function ComboboxList({ className, ...props }: ComboboxPrimitive.List.Props) {
  return (
    <ComboboxPrimitive.List
      ...
      // Eliminamos el cálculo complejo por uno estándar y funcional
      className={cn(
        "max-h-60 overflow-y-auto p-1 overscroll-contain scrollbar-thin",
        className
      )}
      {...props}
    />
